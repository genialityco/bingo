import { v4 } from 'uuid';
import { isBase64Url, uploadBase64ImageToFirebase } from './validationImageExternalUrl';

export const handleAppearanceImageUploads = async (bingoAppearance) => {
  const appearancePromises = Object.entries(bingoAppearance).slice(1).map(async ([clave, valor]) => {
    if (valor !== '') {
